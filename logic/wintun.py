...
def close_adapter(adapter):
    """Closes the adapter handle and removes the interface from the system."""
    if adapter:
        wintun.WintunCloseAdapter(adapter)
        print("[*] Wintun adapter closed and interface removed.")
...
def end_session(session):
    """Stops the Wintun session and clears the ring buffer."""
    if session:
        wintun.WintunEndSession(session)
        print("[*] Wintun session ended.")
